    ...
    with connection.cursor() as cursor:
        if request.method == "POST":
            ...
            if ((num_of_active_loan < borrow_amount_limit) and (num_of_unpaid_fine == 0)):
                ...
                cursor.execute("SELECT copy_ID FROM copy where item_ID = %s and loaned=0 and damaged=0 and lost=0",[lap_model])
                ...
                cursor.execute("UPDATE copy SET loaned=1 WHERE item_ID = %s and copy_ID = %s",[lap_model, copy_to_be_loaned])
                row = cursor.fetchall()
