    ...
    with connection.cursor() as cursor:
        if request.method == "POST":
            ...
            if ((num_of_active_loan < borrow_amount_limit) and (num_of_unpaid_fine == 0)):
                ...
                # create loan details
                cursor.execute("INSERT INTO loan (user_ID, user_type_ID, item_ID, item_copy_ID, item_type, borrow_date, return_due_date, active) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)", [user_ID, user_type, media_ID, copy_to_be_loaned, item_type, today, loan_due_date, 1])
                row = cursor.fetchall()
